...
async function handleSignup(req, res) {
  ...
  try {
    ...
    // Check for duplicate email error from stored procedure
...
async function handleSignin(req, res) {
  ...
  try {
    // Call the stored procedure
    const result = await query(
      'CALL sp_UserSignin(?, @userId, @firstName, @userPassword)',
      [email]
    );
    ...
    const firstName = outputResult[0].firstName;
